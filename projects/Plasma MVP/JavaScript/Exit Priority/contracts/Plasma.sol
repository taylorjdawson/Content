...
contract Plasma {
  ...
  function addExitToQueue(uint256 _utxoPos, address payable _exitor, uint256 _amount, uint256 _created_at)
    public
  {
    require(_amount > 0);
    require(exits[_utxoPos].amount == 0);
    
    uint creationWeeks = _created_at + 2 weeks;
    uint oneWeek = block.timestamp + 1 weeks;
    uint exitableAt = (creationWeeks < oneWeek) ? oneWeek : creationWeeks;
